...
class MonsterData(fileName: String) : Excel(fileName)
{
	override fun makeChangeList()
	{
		...
		for (i in 0..monsters.models.size - 1)
		{
			...
			rowData.add(RowData(defaultStyle, mutableListOf("HP", CompareValues(Dm.hp, m.hp), "Boss Bit", CompareValues(Dm.runFlag, m.runFlag), CompareValues(DspellNames[index], spellNames[index]), CompareValues(DspellChances[index], spellChances[index]), CompareValues(Pget(DitemDropNames, index), Pget(itemDropNames, index++)))))
